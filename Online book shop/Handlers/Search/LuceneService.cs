...
namespace Online_book_shop.Handlers.Search
{
    ...
    public class LuceneService 
    {
        public static class LuceneSearch
        {
            ...
            private static void _addToLuceneIndex(SearchData Data, IndexWriter writer)
            {
                ...
                doc.Add(new Field("ObjectType", Data.ObjectType.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
                doc.Add(new Field("ObjectId", Data.objectId.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
                doc.Add(new Field("Title", Data.Title, Field.Store.YES, Field.Index.ANALYZED));
                doc.Add(new Field("LocalTitle", Data.LocalTitle, Field.Store.YES, Field.Index.ANALYZED));
                doc.Add(new Field("Description", Data.Description, Field.Store.YES, Field.Index.ANALYZED));
                doc.Add(new Field("ISBN", (Data.ISBN !=null? Data.ISBN:""), Field.Store.YES, Field.Index.ANALYZED));

                // add entry to index
                writer.AddDocument(doc);
            }
            public static void AddUpdateLuceneIndex(IEnumerable<SearchData> Datas)
            {
                // init lucene
                ...
                using (var writer = new IndexWriter(_directory, analyzer, IndexWriter.MaxFieldLength.UNLIMITED))
                {
                    ...
                    foreach (var sampleData in Datas) _addToLuceneIndex(sampleData, writer);
           ...
           // LuceneSearch.AddUpdateLuceneIndex(new_record); 
            ...
            public static void AddUpdateLuceneIndex(SearchData Data)
            {
                AddUpdateLuceneIndex(new List<SearchData> {Data });
            }
            ...
            private static SearchData _mapLuceneDocumentToData(Document doc)
            {
                return new SearchData
                {
                    ...
                    ObjectType = Convert.ToInt32(doc.Get("ObjectType")),                   
                    objectId = Convert.ToInt32(doc.Get("ObjectId")),
                    Title = doc.Get("Title"),
                    LocalTitle=doc.Get("LocalTitle"),
                ...
                using (var searcher = new IndexSearcher(_directory, false))
                {
                    ...
                    else
                    {
                        var parser = new MultiFieldQueryParser
                            (Lucene.Net.Util.Version.LUCENE_30, new[] { "Id", "ObjectType", "ObjectId", "Description", "Title", "LocalTitle", "ISBN" }, analyzer);
                        var query = parseQuery(searchQuery, parser);
