...
namespace Online_book_shop.Controllers
{
    ...
    public class AccountController : Controller
    {
            ...
            using (var context = new ApplicationDbContext())
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        PrintedNotification.AccountCreationNotification(user);
                        //return RedirectToAction("Index", "Home");
                        return RedirectToLocal(String.IsNullOrEmpty(returnUrl)?"/":returnUrl);
                    }
                    else
                    {
                        AddErrors(result);
        ...
        public ActionResult UserProfile()
        {
            ...
            var books = BusinessHandlerUserReviews.GetWhistListByUser(BusinessHandlerAuthor.GetLoginUserId());
            ViewBag.NumberOfBooks = books != null ? books.Count : 0;
            ViewBag.Books = books !=null?books.OrderByDescending(x => x.Id).Skip(12 * (page - 1)).Take(12).ToList():null;
            ViewBag.Orders = BusinessHandlerOrder.GerOrderForLogingUser();
            ViewBag.User = BusinessHandlerUser.GetApplicationUser();
